customize_rprocess_M4 <- function(code_rprocess, U, units_all, num_of_strains = 4, N_introdution, id_intro){
  ...
  # The outbreak is introduced in the id_intro unit at the beginning
  stopifnot(id_intro %in% (seq_len(U)-1)) # 0-based index
  code_rprocess <- gsub("u==ZA_code", paste0("u==", id_intro), code_rprocess, fixed=T)
  ...
  ## not introducing BA.2 cases
  code_rprocess <- gsub("day==day_BA_two_int", "0", code_rprocess, fixed=T)
  ...
  code_sum_C_k_i_infected <- paste0(paste0(paste0("C_", seq_len(num_of_strains), "_i_infected_origin_"), rep(seq_len(U)-1, each=4)), "_new[u]") %>% paste0(collapse = " + ")
  code_rprocess <- gsub("code_sum_C_k_i_infected_TO_BE_REPLACED", code_sum_C_k_i_infected, code_rprocess, fixed=T)
  
  code_C_k_i_detected_origin_o_new_IDRover1 <- paste0(paste0(paste0("C_", seq_len(num_of_strains), "_i_detected_origin_"), rep(seq_len(U)-1, each=4)), "_new[u] = ", paste0(paste0("C_", seq_len(num_of_strains), "_i_infected_origin_"), rep(seq_len(U)-1, each=4)), "_new[u];") %>% paste0(collapse = " ")
  code_rprocess <- gsub("code_C_k_i_detected_origin_o_new_IDRover1_TO_BE_REPLACED", code_C_k_i_detected_origin_o_new_IDRover1, code_rprocess, fixed=T)

  code_sum_C_k_i_detected <- paste0(paste0(paste0("C_", seq_len(num_of_strains), "_i_detected_origin_"), rep(seq_len(U)-1, each=4)), "_new[u]") %>% paste0(collapse = " + ")
  code_rprocess <- gsub("code_sum_C_k_i_detected_TO_BE_REPLACED", code_sum_C_k_i_detected, code_rprocess, fixed=T)

  code_C_k_i_detected_origin_o_new_IDRlower1 <- paste0(paste0(paste0("C_", seq_len(num_of_strains), "_i_detected_origin_"), rep(seq_len(U)-1, each=4)), "_new[u] = ", paste0(paste0("C_", seq_len(num_of_strains), "_i_infected_origin_"), rep(seq_len(U)-1, each=4)), "_new[u]*infection_detection_new_u;") %>% paste0(collapse = " ")
  code_rprocess <- gsub("code_C_k_i_detected_origin_o_new_IDRlower1_TO_BE_REPLACED", code_C_k_i_detected_origin_o_new_IDRlower1, code_rprocess, fixed=T)

  code_C_k_i_detected_origin_o_new_newIDRlower1 <- paste0(paste0(paste0("C_", seq_len(num_of_strains), "_i_detected_origin_"), rep(seq_len(U)-1, each=4)), "_new[u] = ", paste0(paste0("C_", seq_len(num_of_strains), "_i_infected_origin_"), rep(seq_len(U)-1, each=4)), "_new[u]*new_IDR_i_u;") %>% paste0(collapse = " ")
  code_rprocess <- gsub("code_C_k_i_detected_origin_o_new_newIDRlower1_TO_BE_REPLACED", code_C_k_i_detected_origin_o_new_newIDRlower1, code_rprocess, fixed=T)

  code_C_k_i_sequenced_origin_o_new_DSRover1 <- paste0(paste0(paste0("C_", seq_len(num_of_strains), "_i_sequenced_origin_"), rep(seq_len(U)-1, each=4)), "_new[u] = ", paste0(paste0("C_", seq_len(num_of_strains), "_i_detected_origin_"), rep(seq_len(U)-1, each=4)), "_new[u];") %>% paste0(collapse = " ")
  code_rprocess <- gsub("code_C_k_i_sequenced_origin_o_new_DSRover1_TO_BE_REPLACED", code_C_k_i_sequenced_origin_o_new_DSRover1, code_rprocess, fixed=T)

  code_sum_C_k_i_sequenced <- paste0(paste0(paste0("C_", seq_len(num_of_strains), "_i_sequenced_origin_"), rep(seq_len(U)-1, each=4)), "_new[u]") %>% paste0(collapse = " + ")
  code_rprocess <- gsub("code_sum_C_k_i_sequenced_TO_BE_REPLACED", code_sum_C_k_i_sequenced, code_rprocess, fixed=T)

  code_C_k_i_sequenced_origin_o_new_DSRlower1 <- paste0(paste0(paste0("C_", seq_len(num_of_strains), "_i_sequenced_origin_"), rep(seq_len(U)-1, each=4)), "_new[u] = ", paste0(paste0("C_", seq_len(num_of_strains), "_i_detected_origin_"), rep(seq_len(U)-1, each=4)), "_new[u]*detection_sequencing_new_u;") %>% paste0(collapse = " ")
  code_rprocess <- gsub("code_C_k_i_sequenced_origin_o_new_DSRlower1_TO_BE_REPLACED", code_C_k_i_sequenced_origin_o_new_DSRlower1, code_rprocess, fixed=T)

  code_C_k_i_sequenced_origin_o_new_newDSRlower1 <- paste0(paste0(paste0("C_", seq_len(num_of_strains), "_i_sequenced_origin_"), rep(seq_len(U)-1, each=4)), "_new[u] = ", paste0(paste0("C_", seq_len(num_of_strains), "_i_detected_origin_"), rep(seq_len(U)-1, each=4)), "_new[u]*new_DSR_i_u;") %>% paste0(collapse = " ")
  code_rprocess <- gsub("code_C_k_i_sequenced_origin_o_new_newDSRlower1_TO_BE_REPLACED", code_C_k_i_sequenced_origin_o_new_newDSRlower1, code_rprocess, fixed=T)
