...
for (u = 0; u < U; u++) {
  ...
  // strain 1
  m = C_1_c_sequenced_new[u];
  if (m > 0.0) {
    v = m*tau_seq_unit[u*tau_seq_unit_unit] > 1 ? m*tau_seq_unit[u*tau_seq_unit_unit] : 1;
    C_1_c[u] = nearbyint(rnorm(m, sqrt(v)));
  } else {
    C_1_c[u] = 0.0;
  }
  // strain 2
  m = C_2_c_sequenced_new[u];
  if (m > 0.0) {
    v = m*tau_seq_unit[u*tau_seq_unit_unit] > 1 ? m*tau_seq_unit[u*tau_seq_unit_unit] : 1;
    C_2_c[u] = nearbyint(rnorm(m, sqrt(v)));
  } else {
    C_2_c[u] = 0.0;
  }
  // strain 3
  m = C_3_c_sequenced_new[u];
  if (m > 0.0) {
    v = m*tau_seq_unit[u*tau_seq_unit_unit] > 1 ? m*tau_seq_unit[u*tau_seq_unit_unit] : 1;
    C_3_c[u] = nearbyint(rnorm(m, sqrt(v)));
  } else {
    C_3_c[u] = 0.0;
  }
  // strain 4
  m = C_4_c_sequenced_new[u];
  if (m > 0.0) {
    v = m*tau_seq_unit[u*tau_seq_unit_unit] > 1 ? m*tau_seq_unit[u*tau_seq_unit_unit] : 1;
    C_4_c[u] = nearbyint(rnorm(m, sqrt(v)));
  } else {
    C_4_c[u] = 0.0;
  }

  // daily sequenced cases imported, N=116
  code_rmeasure_C_k_i_o_TO_BE_REPLACED;
} 
