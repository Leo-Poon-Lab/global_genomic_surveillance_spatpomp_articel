# This code builds the model M_0 in the paper

# This script is for building the pomp model for calibration #
# There are some essential parts: #
# 0. Sanity check of the input parameters #
...
build_global_spatPomp_Omicron20 <- function(
  global_data,
  dt_years = 1 / 365, 
  ...
  N_introdution,
  for_IBPF = FALSE # we don't use IBPF for searching the shared parameters, but for the unit-specific parameters. Setting it to FALSE will ensure that the shared parameters only refer to the first unit.
) {
  # 3.3.0. Sanity check #
  ...
  code_rprocess_tail_M0 <- paste0(readLines("scripts/model_building/helper/code_rprocess_tail_M0.C"), collapse = "\n") # load code_rprocess_tail_M0, this will be changed in the model for simulation
  code_rprocess <- paste0(code_rprocess_main_M0, code_rprocess_tail_M0)

  source("scripts/model_building/helper/customize_rprocess_M0.R")
  code_rprocess <- customize_rprocess_M0(code_rprocess, U, units_all, num_of_strains, N_introdution)
  ...
  saveRDS(data_measurements, file = here::here("results/model_data/data_measurements_Omicron20.rds"))
  saveRDS(data_covariates, file = here::here("results/model_data/data_covariates_Omicron20.rds"))
